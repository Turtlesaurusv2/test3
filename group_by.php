...
$results = $conn->prepare("SELECT  pd_c, SUM(quantity) as quantity,sub_id FROM product WHERE sub_id = $sub_id GROUP BY pd_c ");
$results->execute();
...
if($results){
    ...
    $stmt = $conn->prepare("INSERT INTO prod (pd_c ,quantity,sub_id)  SELECT  pd_c, SUM(quantity),sub_id as quantity FROM product WHERE sub_id = $sub_id GROUP BY pd_c "); 
    $stmt->execute();
...
if($stmt){
    ...
    if($resu){
        ...
        $stmt2 = $conn->prepare("INSERT INTO product (pd_c ,quantity,sub_id)  SELECT  pd_c, quantity,sub_id  FROM prod "); 
        $stmt2->execute();
