...
$sub_id = $data['sub_id'];
    ...
    $result2 = $connect->prepare("SELECT * FROM inventory_summary  WHERE  product_id = '$pd_c' LIMIT 0,10 ");
    ...
    if($pd_c == $inv ){
        ...
        $stmt = $conn->prepare("INSERT INTO product SET pd_c = :pd_c, quantity = :quantity, sub_id = :sub_id"); 
        $stmt->execute(["pd_c"=> $pd_c, ":quantity" => $quantity, ":sub_id" => $sub_id]);
